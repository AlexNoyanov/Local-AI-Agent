...
function App() {
  ...
  useEffect(() => {
    fetch('http://localhost:4891/models')
      .then(res => res.json())
      .then(setModels);
  }, []);
  ...
  const fetchDocuments = () => {
    fetch('http://localhost:4891/documents')
      .then(res => res.json())
      .then(setDocuments);
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const res = await fetch('http://localhost:4891/chat', {
      method: 'POST',
  ...
  const handleModelChange = async (model) => {
    ...
    await fetch('http://localhost:4891/model', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleFileUpload = async (e) => {
    ...
    await fetch('http://localhost:4891/upload', {
      method: 'POST',
      body: formData
    });
  ...
  const setNewUploadPath = async () => {
    await fetch('http://localhost:4891/set-upload-path', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
